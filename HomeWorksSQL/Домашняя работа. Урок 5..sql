...
/*
2. Найдите кол-во, отправленных сообщений каждым пользователем и выведите ранжированный список пользователь, указав указать имя 
и фамилию пользователя, количество отправленных сообщений и место в рейтинге (первое место у пользователя с максимальным количеством сообщений) . 
(используйте DENSE_RANK)
*/

SELECT u.id,
       CONCAT(firstname, ' ', lastname) AS 'Имя и фамилия пользователя',
       COUNT(m.id) AS 'Количество отправленных сообщений',
       DENSE_RANK() OVER (ORDER BY COUNT(m.id) DESC) AS 'Место в рейтинге'
FROM users u
JOIN messages m ON u.id = m.from_user_id
GROUP BY u.id;
-- Тех, кто не написал ни одного сообщения нет

/*
3. Выберите все сообщения, отсортируйте сообщения по возрастанию даты отправления (created_at) и найдите разницу дат отправления между 
соседними сообщениями, получившегося списка. (используйте LEAD или LAG)
*/

SELECT m.*,
	   TIMESTAMPDIFF(SECOND, created_at, LEAD(created_at) OVER (ORDER BY created_at)) AS 'Разница в секундах',
